...
void CJumpPoint::InitTile(void)
{
	...
	for (int iCnt = 0; iCnt < dfLIST_MAX; ++iCnt)
	{
		if (nullptr == m_pOpenList[iCnt])
			break;

		m_byTileAttribute[m_pOpenList[iCnt]->iY][m_pOpenList[iCnt]->iX] = df_NONBLOCK;
		delete m_pOpenList[iCnt];
		m_pOpenList[iCnt] = nullptr;
	}
	
	for (int iCnt = 0; iCnt < dfLIST_MAX; ++iCnt)
	{
		if (nullptr == m_pCloseList[iCnt])
			break;

		m_byTileAttribute[m_pCloseList[iCnt]->iY][m_pCloseList[iCnt]->iX] = df_NONBLOCK;
		delete m_pCloseList[iCnt];
		m_pCloseList[iCnt] = nullptr;
	}
...
void CJumpPoint::MakeStartNode(int iStartX, int iStartY, int iEndX, int iEndY)
{
	...
	m_pOpenList[m_iOpenListNum] = stpNew;
	++m_iOpenListNum;
...
{
	st_TILENODE *stpTempNode = nullptr;
	...
	stpTempNode = m_pOpenList[0];
	m_pCloseList[m_iCloseListNum] = m_pOpenList[0];
	++m_iCloseListNum;
	memcpy_s(&m_pOpenList, sizeof(st_TILENODE *) * dfLIST_MAX, &m_pOpenList[1], sizeof(st_TILENODE *) * (m_iOpenListNum - 1));
	m_pOpenList[m_iOpenListNum - 1] = nullptr;
	--m_iOpenListNum;
...
bool CJumpPoint::LoadGameMap(WCHAR *szFilePath)
{
	...
	//{
	//	// 'X,' 또는 ' X'
	//	if (m_wpBuffer[iBufPos] == 0x2c58 || m_wpBuffer[iBufPos] == 0x5820)
	//	{
	//		// 블럭
	...
	//	}
	//	// 'X.' X 후, 개행
	//	else if (m_wpBuffer[iBufPos] == 0x0a58)
	//	{
	//		iX = iX;
	//		iY = iY;
...
void CJumpPoint::CheckNeighbour(st_TILENODE *stpCurrent)
{
	...
	// 1. 벡터 생성
	float VectorX = m_iEndX - iCurrentX;
	float VectorY = m_iEndY - iCurrentY;
...
BOOL CJumpPoint::MakeNode(st_TILENODE *stpCurrent, int iCurrentX, int iCurrentY, int iDir)
{
	...
	if (JumpNode(iCurrentX, iCurrentY, &iJumpX, &iJumpY, stpCurrent->fG + 1, &fJumpG, iDir))
	{
		...
		while (1)
		{
			if (m_iOpenListNum == iListIdx)
				break;

			if (m_pOpenList[iListIdx]->iX == iJumpX && m_pOpenList[iListIdx]->iY == iJumpY)
			{
				if (m_pOpenList[iListIdx]->fG > fJumpG)
				{
					m_pOpenList[iListIdx]->fG = fJumpG;
					m_pOpenList[iListIdx]->stpParent = stpCurrent;
				}
		...
		while (1)
		{
			if (m_iCloseListNum == iListIdx)
				break;

			if (m_pCloseList[iListIdx]->iX == iJumpX && m_pCloseList[iListIdx]->iY == iJumpY)
			{
				if (m_pCloseList[iListIdx]->fG > fJumpG)
				{
					m_pCloseList[iListIdx]->fG = fJumpG;
					m_pCloseList[iListIdx]->stpParent = stpCurrent;
				}
		...
		while (1)
		{
			if (iListIdx == m_iOpenListNum)
			{
				m_pOpenList[iListIdx] = stpNew;
				++m_iOpenListNum;
				break;
			}
			else if (m_pOpenList[iListIdx]->fF >= stpNew->fF)
			{
				memmove_s(&m_pOpenList[iListIdx + 1], sizeof(st_TILENODE *) * (dfLIST_MAX - (iListIdx + 1)), &m_pOpenList[iListIdx], sizeof(st_TILENODE *) * (m_iOpenListNum - iListIdx));
				//memcpy_s(&m_pOpenList[iListIdx + 1], sizeof(st_TILENODE *) * (dfLIST_MAX - (iListIdx + 1)), &m_pOpenList[iListIdx], sizeof(st_TILENODE *) * (m_iOpenListNum - iListIdx));
				m_pOpenList[iListIdx] = stpNew;
				++m_iOpenListNum;
				break;
			}
...
BOOL CJumpPoint::JumpNode(int iCurrentX, int iCurrentY, int *ipJumpX, int *ipJumpY, float fCurrentG, float *fpJumpG, int iDir)
{
	...
	{
	case df_UU:
		if (TRUE == JumpNode_UU(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
	case df_UR:
		if (TRUE == JumpNode_UR(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
	case df_RR:
		if (TRUE == JumpNode_RR(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
	case df_RD:
		if (TRUE == JumpNode_RD(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
	case df_DD:
		if (TRUE == JumpNode_DD(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
	case df_DL:
		if (TRUE == JumpNode_DL(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
	case df_LL:
		if (TRUE == JumpNode_LL(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
	case df_LU:
		if (TRUE == JumpNode_LU(iCurrentX, iCurrentY, ipJumpX, ipJumpY, fCurrentG, fpJumpG))
			return TRUE;
		else
			return FALSE;
		break;
...
BOOL CJumpPoint::JumpNode_UU(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iDepth = dfDEPTH_MAX;

	//--------------------------------------------------------------------------
	// 상하좌우는 동일한 방향으로 계속 진행
	//--------------------------------------------------------------------------
	while (1)
	{
		// 해당 타일 검사
...
BOOL CJumpPoint::JumpNode_UR(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iTempX1, iTempX2;
	int iTempY1, iTempY2;
	float fTempG1, fTempG2;
	...
	while (1)
	{
		iTempX1 = iTempX2 = iCurrentX;
		iTempY1 = iTempY2 = iCurrentY;
		fTempG1 = fTempG2 = fCurrentG;
		...
		while (1)
		{
			...
			// 해당 좌표에 0~9 사이의 임의 상수 값 저장
			//--------------------------------------------------------------------------
			if ((iTempX1 != m_iStartX || iTempY1 != m_iStartY) && (iTempX1 != m_iEndX || iTempY1 != m_iEndY))
				m_byTileAttribute[iTempY1][iTempX1] = m_iJumpIdx;

			//--------------------------------------------------------------------------
			// 현재 좌표가 끝지점이라면 값 세팅하고 리턴
			//--------------------------------------------------------------------------
			if (iTempX1 == m_iEndX && iTempY1 == m_iEndY)
			{
				*ipJumpX = iCurrentX;
				*ipJumpY = iCurrentY;
				*fpJumpG = fCurrentG;
			...
			// 꺽임 검사 UU
			if ((CHECK_TILE(iTempX1 - 1, iTempY1 - 1) & (m_byTileAttribute[iTempY1][iTempX1 - 1] == df_BLOCK) & (m_byTileAttribute[iTempY1 - 1][iTempX1 - 1] == df_NONBLOCK))
				| (CHECK_TILE(iTempX1 + 1, iTempY1 - 1) & (m_byTileAttribute[iTempY1][iTempX1 + 1] == df_BLOCK) & (m_byTileAttribute[iTempY1 - 1][iTempX1 + 1] == df_NONBLOCK)))
			{
				*ipJumpX = iCurrentX;
				*ipJumpY = iCurrentY;
				*fpJumpG = fCurrentG;
		...
		while (1)
		{
			...
			// 해당 좌표에 0~9 사이의 임의 상수 값 저장
			//--------------------------------------------------------------------------
			if ((iTempX2 != m_iStartX || iTempY2 != m_iStartY) && (iTempX2 != m_iEndX || iTempY2 != m_iEndY))
				m_byTileAttribute[iTempY2][iTempX2] = m_iJumpIdx;

			//--------------------------------------------------------------------------
			// 현재 좌표가 끝지점이라면 값 세팅하고 리턴
			//--------------------------------------------------------------------------
			if (iTempX2 == m_iEndX && iTempY2 == m_iEndY)
			{
				*ipJumpX = iCurrentX;
				*ipJumpY = iCurrentY;
				*fpJumpG = fCurrentG;
			...
			// 꺽임 검사 RR
			if ((CHECK_TILE(iTempX2 + 1, iTempY2 - 1) & (m_byTileAttribute[iTempY2 - 1][iTempX2] == df_BLOCK) & (m_byTileAttribute[iTempY2 - 1][iTempX2 + 1] == df_NONBLOCK))
				| (CHECK_TILE(iTempX2 + 1, iTempY2 + 1) & (m_byTileAttribute[iTempY2 + 1][iTempX2] == df_BLOCK) & (m_byTileAttribute[iTempY2 + 1][iTempX2 + 1] == df_NONBLOCK)))
			{
				*ipJumpX = iCurrentX;
				*ipJumpY = iCurrentY;
				*fpJumpG = fCurrentG;
...
BOOL CJumpPoint::JumpNode_RR(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iDepth = dfDEPTH_MAX;
...
BOOL CJumpPoint::JumpNode_RD(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iTempX1, iTempX2;
	int iTempY1, iTempY2;
	float fTempG1, fTempG2;
	...
	while (1)
	{
		...
		while (1)
		{
			...
			// 꺾임 확인 RR
			if ((CHECK_TILE(iTempX1 + 1, iTempY1 - 1) & (m_byTileAttribute[iTempY1 - 1][iTempX1] == df_BLOCK) & (m_byTileAttribute[iTempY1 - 1][iTempX1 + 1] == df_NONBLOCK))
				| (CHECK_TILE(iTempX1 + 1, iTempY1 + 1) & (m_byTileAttribute[iTempY1 + 1][iTempX1] == df_BLOCK) & (m_byTileAttribute[iTempY1 + 1][iTempX1 + 1] == df_NONBLOCK)))
			{
				*ipJumpX = iCurrentX;
				*ipJumpY = iCurrentY;
				*fpJumpG = fCurrentG;
		...
		while (1)
		{
			...
			// 꺾임 확인 DD
			if ((CHECK_TILE(iTempX2 - 1, iTempY2 + 1) & (m_byTileAttribute[iTempY2][iTempX2 - 1] == df_BLOCK) & (m_byTileAttribute[iTempY2 + 1][iTempX2 - 1] == df_NONBLOCK))
				| (CHECK_TILE(iTempX2 + 1, iTempY2 + 1) & (m_byTileAttribute[iTempY2][iTempX2 + 1] == df_BLOCK) & (m_byTileAttribute[iTempY2 + 1][iTempX2 + 1] == df_NONBLOCK)))
			{
				*ipJumpX = iCurrentX;
				*ipJumpY = iCurrentY;
				*fpJumpG = fCurrentG;
...
BOOL CJumpPoint::JumpNode_DD(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iDepth = dfDEPTH_MAX;
...
BOOL CJumpPoint::JumpNode_DL(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iTempX1, iTempX2;
	int iTempY1, iTempY2;
	float fTempG1, fTempG2;
...
BOOL CJumpPoint::JumpNode_LL(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iDepth = dfDEPTH_MAX;
...
BOOL CJumpPoint::JumpNode_LU(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG)
{
	int iTempX1, iTempX2;
	int iTempY1, iTempY2;
	float fTempG1, fTempG2;
...
//BOOL CJumpPoint::JumpNode_Dir(int iCurrentX, int iCurrentY, int * ipJumpX, int * ipJumpY, float fCurrentG, float * fpJumpG, int iDir)
//{
//	float fTempG = 0;
//
//	//--------------------------------------------------------------------------
